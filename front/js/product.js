...
function storeInLocal() {
  ...
  addToCart.addEventListener("click", () => {
    ...
    if ((colorV === "" && quantityV < 1) || quantityV > 100) {
      ...
      window.alert(
        "La quantité maximale est de 100, merci de choisir une quantité inférieure."
      );
      return;
      ...
      if (cart === null) {
        ...
        var foundProduct = cart.find(
          (p) => p.id == custSelect.id && p.color == custSelect.color
        );
        ...
        if (foundProduct != undefined) {
          var newQuantity =
            parseInt(custSelect.quantity) + parseInt(foundProduct.quantity);
